...
async def get_leads(client):
    ...
    if response.status_code == 204:
      return response.status_code
    return str(json.loads(response.text)[0].links)
...
async def post_lead(client, data):
    data = {
       ...
       'pipeline_id': 8412118,
       'custom_fields_values': [ {'field_id': 838641, 'values': [{'value': data.link}]},{'field_id': 923969, 'values': [{'value': data.price * 0.03}]}, {'field_id': 923963, 'values': [{'value': data.address}]}, {'field_id': 923967, 'values': [{'value': data.phone}]}, {'field_id': 923965, 'values': [{'value': data.seller}]}]
...
async def f(lead: Lead):
    start = time()
    output = await task(lead, None, None)
    print("time: ", time() - start)
    return output
